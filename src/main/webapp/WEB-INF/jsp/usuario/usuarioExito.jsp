<html>

<head>
<META HTTP-EQUIV="REFRESH" CONTENT="5;URL=welcome.htm"> 

<script src="resources/metrostyle/jquery/jquery.js"
	type="text/javascript"></script>


<!-- Metro-UI-CSS-master -->
<script src="resources/Metro-UI-CSS-master/jquery.widget.min.js"></script>
<link rel="stylesheet"
	href="resources/Metro-UI-CSS-master/css/metro-bootstrap.css">

<script src="resources/Metro-UI-CSS-master/min/metro.min.js"></script>
<script src="resources/Metro-UI-CSS-master/js/metro-dropdown.js"></script>


</head>

<body class="metro">
	<div class="grid">
		<div class="row">
			<div class="span6 offset4">
				<div class="example">


					<label>El usuario a sido creado con exito. Se
						redireccionara en 5 segundos. </label> <label>Si no redirecciona
						automaticamente haga click <a href="login.htm">aqui</a>.
					</label>


				</div>
</body>

</html>


